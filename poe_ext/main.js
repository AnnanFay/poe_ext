    ...
    for (key in oTypes) {
        if (oTypes.hasOwnProperty(key)) {
            ...
            var list = $('<li><label class="checkbox">' + cbox + key + '' + expandLink + '</label></li>')
                .appendTo(oTypeUL);
            var inner = $('<ul id="view' + key + '" class="collapse" style="list-style: none;"/>')
                .appendTo(list);
...
function renderCrafting(items) {
    ...
    try {
        ...
        $('ul#craftingTabs,div#crafting-content')
            .empty();
        ...
        for (var item in matches) {
            ...
            $('ul#craftingTabs')
                .append('<li class="crafting-page"><a data-index="' + idx + '">' + item + ' (' + matches[item].length + ')</a></li>');
...
function getLocationLink(item, category) {
    ...
    if (item.location.section == 'stash' || item.location.page == 'Inventory') {
        ...
        if ($('#' + category.toLowerCase() + 'Location')
            .find('input[type=checkbox]:checked')
            .prop('id') == 'show' + category + 'LocationTable') {
...
function getLocationTable(item, category) {
    ...
    if (item.location.section == 'stash' || item.location.page == 'Inventory') {
        ...
        for (var i = 0; i < height; i++) {
            var row = $('<div>')
                .addClass('locationTableRow');

            for (var j = 0; j < 12; j++) {
                var cell = $('<div>')
                    .addClass('locationTableCell');
...
function displaySockets(item) {
    ...
    var sockets = $('<div class="sockets" style="width: ' + parseInt(48 * item.rawItem.w, 10) + 'px; height: ' + $('.itemIcon')
        .height() + 'px' + globalOffsetY + '">');
    var icon = {};
...
function formatRareList(sortedRares, bSetupDropdown) {
    ...
    for (var i = 0; i < sortedRares.length; ++i) {
        ...
        for (j = 0; j < oView.props.length; j++) {
            tr.append($('<td>')
                .text(item.properties.hasOwnProperty(oView.props[j]) ? item.properties[oView.props[j]] : ''));
        }
        for (j = 0; j < oView.mods.length; j++) {
            tr.append($('<td>')
                .text(item.combinedMods.hasOwnProperty(oView.mods[j]) ? item.combinedMods[oView.mods[j]] : ''));
        }
        for (j = 0; j < oView.calculated.length; j++) {
            tr.append($('<td>')
                .text(item.calculated.hasOwnProperty(oView.calculated[j]) ? item.calculated[oView.calculated[j]] : ''));
        }
        for (j = 0; j < oView.reqs.length; j++) {
            tr.append($('<td>')
                .text(item.requirements.hasOwnProperty(oView.reqs[j]) ? item.requirements[oView.reqs[j]] : ''));
        }
    ...
    for (j = 0; j < oView.props.length; j++) {
        if (oView.props[j] === 'Base Type') {
            th.append($('<th class="type-string">')
                .text(oView.props[j]));
        } else {
            th.append($('<th class="type-float">')
                .text(oView.props[j]));
        }
    }
    for (j = 0; j < oView.mods.length; j++) {
        th.append($('<th class="type-int">')
            .text(oView.mods[j]));
    }
    for (j = 0; j < oView.calculated.length; j++) {
        th.append($('<th class="type-float">')
            .text(oView.calculated[j]));
    }
    for (j = 0; j < oView.reqs.length; j++) {
        th.append($('<th class="type-int">')
            .text(oView.reqs[j]));
...
function getSortedItems(items) {
    ...
    var min = parseInt($('#inventoryMinLevel')
        .val(), 10);
