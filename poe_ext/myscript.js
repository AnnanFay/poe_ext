...
function itemCategory(baseType) {
    if (baseType in ITEM_TYPE_DATA) {
        return ITEM_TYPE_DATA[baseType];
    }
    if (baseType in CURRENCY_DATA) {
        return CURRENCY_DATA[baseType];
    }
...
function getSocketLinkage(itemDiv) {
    ...
    var aSockets = $(itemDiv)
        .find('div.sockets img[src$="Socket_Link_Horizontal.png"], div.sockets img[src$="Socket_Link_Vertical.png"]');
    ...
    aSockets.each(function(index, item) {
        ...
        var coords = oLink.attr('style')
            .replace(/^.*top:(\d+).*left:(\d+).*$/, '($1,$2)')
...
var negetives = ["reduced", "reduced maximum"]
    ...
    function itemLevel(item) {
        ...
        if (item.requirements.hasOwnProperty('Required Level')) {
            return parseInt(item.requirements['Required Level'], 10);
        }
        if (item.properties.hasOwnProperty('Required Level')) {
            return parseInt(item.properties['Required Level'], 10);
        }
